...
def insert(node, key, parent):
    ...
    #### 결국 argument으로 받아온 node를 그대로 반환한다. ####
    return node
...
def delete_node(node):
    ...
    # 현재 삭제할 node에 자식 노드가 2개인 경우
        ...
        # 현재 node.left를 임시로 담음
        temp = node.left
        
        # 해당 right에서 right만을 바라보며 최댓값 가져오기
        ...
        # 현재 temp는 최댓값을 가진 node이다.
        # node를 삭제하는 것 대신에, node.key에 temp.key(최댓값)을 대입한다.
        node.key = temp.key
        ...
        if temp == node.left:
            ...
            # key는 이미 위의 코드에서 처리했으므로 left도 가져온다.
            node.left = temp.left
        else:
